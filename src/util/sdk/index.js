...
export default {
  ...
  async setApprovalForAll(asset, coinbase, operator, approved){
    ...
    try{
      return await contract.setApprovalForAll(operator, approved, {from: coinbase});
    }catch(e){
      return {error: e.message}
    }
  },
  ...
  async getAssetContract(asset){
    ...
    return await this.contractAt(asset, asset.abi);
  },
  async transferAsset(asset, from, to){
    ...
    if(contract.error) return contract
    try{
      return await contract.transferFrom(from, to, asset.tokenId, {from: from});
    }catch(e){
      return { error: e.message }
    }
  },
  ...
  async getExchangeContract(asset){
    let abi = utils.contractAbi("EXCHANGE");
    asset.contractAddress = asset.exchangeAddress;
    return await this.contractAt(asset, abi);
  },
  async contractAt(asset, abi){
