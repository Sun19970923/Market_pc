    ...
    <div class="bg-box"
      ...
      <div class="head-portrait">
        <avatar class="u-info-avatar" :imageUrl="$filters.fullImageUrl(contract.cover)"
          :address="contract.address" :imgWidth="88" :imgHeight="88" shape="circular"></avatar>
      </div>
    </div>
    <div class="user-information u-info-left">
      <div class="u-info-name" v-if="contract.name">{{ contract.name }}</div>
      <div class="u-info-address justify-center">
        <span>{{ showAddress }}</span>
        <span class="u-info-copy iconfont icon-copy" v-clipboard:copy="contract.address"
          v-clipboard:success="onSuccessCopy" v-clipboard:error="onErrorCopy"></span>
      </div>
      <div class="u-info-introduction">
        {{ contract.description || "This is an introduction." }}
      </div>
    </div>
          ...
          <el-tab-pane class="tab-content-info" v-for="(tab,i) in tabs" :key="i" :name="tab.name">
            ...
            <div class="new-nft-list" v-infinite-scroll="loadList">
              <div class="tab-content-none"
                v-if="!nftList[tab.name].length && loadStatus != 'loading'">
                <div class="tab-info">
              ...
              <template v-else>
                <div class="nft">
                  <nft-item v-for="(nft, i) in nftList[tab.name]" :nft="nft" :index="i"
